...
function submitProductForm(event) {
    ...
    let { title, status, category, description, price, thumbnail, stock } = Object.fromEntries(formData);

    status = status === "on" ? true : false;
    price = parseFloat(price);
    stock = parseInt(stock, 10);

    socket.emit("addProduct", {title, status, category, description, price, thumbnail,  stock});
}
